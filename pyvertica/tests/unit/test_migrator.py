...
class VerticaMigratorTestCase(unittest.TestCase):
    ...
    def test___init(self, get_connection, truncate_table):
        """
        ...
        # variables
        self.assertEqual('SourceDSN', migrator._source)
        self.assertEqual('TargetDSN', migrator._target)
        self.assertEqual(False, migrator._commit)
        self.assertEqual({}, migrator._args)

        # db connection
        get_connection.assert_called_once_with('SourceDSN')
        get_connection.assert_called_once_with('TargetDSN')
